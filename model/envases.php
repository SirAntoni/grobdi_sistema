...
class Envases extends Conectar
{
    ...
    public function get_envases_paginacion($pagina)
    {       
            $pagina = is_null($pagina) ? 1 : (int)$pagina;
            $regPagina = 15;
            $inicio = ($pagina > 1) ? (($pagina * $regPagina)) - $regPagina : 0;
            
            $registros = "SELECT SQL_CALC_FOUND_ROWS * FROM insumos i INNER JOIN articulos a ON i.codigo_sku_insumo = a.codigo_sku  WHERE i.codigo_interno_insumo LIKE '%EN%' GROUP BY i.codigo_interno_insumo ORDER BY i.id DESC LIMIT :inicio,:total";
